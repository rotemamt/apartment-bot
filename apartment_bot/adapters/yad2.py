...
class Yad2Adapter(SourceAdapter):
    ...
    async def _fetch_listings_async(self, known_urls: set[str]) -> list[Listing]:
        ...
        # sandbox=False: Chrome refuses to start sandboxed when run as root
        # (the case inside our Docker container); harmless for a non-root
        # local run too, so use it unconditionally rather than branching.
        # --disable-dev-shm-usage: Docker's default 64MB /dev/shm is too
        # small for Chrome under real memory pressure and causes silent
        # crashes; this makes Chrome use /tmp instead. Belt-and-suspenders
